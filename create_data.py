...
def create_users(db, num):
    ...
    for _ in range(num):
        users.append({
            ...
            "registration_date": datetime.now(timezone.utc) - timedelta(days=random.randint(0, 365))
        })
...
def get_products(db):
    ...
    for d in data:
        d['creation_date']=datetime.now(timezone.utc)
        d['gender']="unisex"
...
def create_posts(db, users, num):
    ...
    normal_users = [u for u in users if u['type'] == 'user']
    ...
    for _ in range(num):
        ...
        posts.append({
            ...
            "creation_date": datetime.now(timezone.utc) - timedelta(days=random.randint(0, 180)),
